...
\newcommand{\gray}[1]{\Large \textcolor{gray}{#1} \normalsize}
\newcommand{\olive}[1]{\Large \textcolor{olive}{\textbf{#1}}\normalsize}
\newcommand{\brown}[1]{\Large \textcolor{brown}{\textit{#1}}\normalsize}
\newcommand{\other}[1]{\Large \textcolor{orange}{\textit{#1}}\normalsize}
...
\subsection{Data}
We created our own data set with help from Amazon Mechanical Turk (mTurk). The data set consists of pairs question-label with questions on news events from past year on politics, sport and stock market. The original data set consisted of \textasciitilde 250 questions and was expanded with auto-generated questions on big sporting event winners (Did team A win event X?) and various US election results. The currently used data set has \textasciitilde 2000 questions.

We would certainly benefit from expanding the data set further, particularly because we do not yet have enough data for a three-way split (train-validation-test) and are only using train-validation split.


\subsection{Syphon}
We picked \textit{spacy} as the dependency parser of choice. Spacy is one of the fastest dependency parsers available  (14000 tokens/s), while maintaining good accuracy.\footnote{http://aclweb.org/anthology/P/P15/P15-1038.pdf}
...
\gray{Will} \olive{Hillary Clinton} \brown{run} \gray{for} \olive{president}\gray{?}

\gray{Will the} \olive{Patriots} \brown{win} \gray{the} \other{2015} \olive{Super Bowl}\gray{?}

\gray{Will} \olive{gay marriage} \gray{be} \brown{legalized} \gray{in} \other{2015}\gray{?}
...
Will the Patriots win the 2015 Super Bowl?
