...
set and a \textit{testing} set (even finee splits are often desirable
...
\footnote{The \textit{word2vec} method uses an idea called \textit{multi-task learning}
	which may be useful for us as well --- that we are trying to learn
...
the caveat of which is the requirement of an extensively labelled data set.
...
and entailment.  The primary approach for debugging initial version
