...
...smh more about spacy?...

We use spacy to look for keywords in the posed question. 
...
\[ y  = \dfrac{\sum_i \sigma(wc_i)\sigma(qr_i)}{\sum_i \sigma(qr_i)}
\]
...
$$a(wc_i) \in  \left\langle 0,1\right\rangle , \quad a(qr_i) > 0$$
...
\item\textbf{Verb similarity:} Because we do not search the database for verb in the question itself, we introduce verb similarity features. These features output a number $\in \left\langle 0,1\right\rangle$ capturing linguistical similarity of those words. We use WordNet and spacy, each with its own feature.
\end{itemize}
...
---how rnn works? (not necessary)---

The outputs of the RNN are then sent side by side to a simple dense layer, wich oitputs a single scalar (vector) representing its value.
